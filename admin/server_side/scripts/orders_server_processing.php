<?php
	 ...
	 */
	$aColumns = array( 'id', 'order_id', 'firstname', 'email', 'amount', 'title', 'qty', 'price', 'item_id', 'thumbnail','ad_option', 'email', 'phone', 'address1', 'zipcode', 'status','order_date' );
	...
	while ( $aRow = mysql_fetch_array( $rResult ) )
	{
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			if ( $aColumns[$i] != ' ' && $aColumns[$i] != 'item_id' && $aColumns[$i] != 'email' && $aColumns[$i] != 'thumbnail' )
			{
				/* General output */
				$row[] = $aRow[ $aColumns[$i] ];
			}
