...
function setCookie(e) {
    $.cookie("cart", JSON.stringify(e), { path: '/' })
}
...
function updateCart() {
    ...
    var t = $("#sub-cart"),
        n = t.find(".cart-items"),
        r = t.find(".cart-header"),
        i = t.find(".cart-total .total");

    n.empty();
    r.find("small").hide();
    if (typeof e == "undefined") {
        r.find("span").text("Your cart is currently empty.");
        i.text("0");
        return
    } else {
        var s = 0,
        ...
        for (var f in e) {
            ...
            if (++o > a) continue;
            var l = $('<li> 							<div class="item clearfix" data-product-id="' + e[f].id + '"> 								<button type="button" class="close" aria-hidden="true">×</button> 								<a href="' + e[f].thumbnail + '" data-toggle="lightbox" class="entry-thumbnail"> 									<img src="' + e[f].thumbnail + '" alt="' + e[f].title + '" /> 								</a> 								<h5 class="entry-title"><a href="' + e[f].url + '">' + e[f].title + '</a></h5> 								<span class="entry-price">' + e[f].qty + " x " + "₹" +  e[f].price + "<code style='margin-left:50px'>" + sample + "</code> </span> 							</div> 						</li>");
            l.appendTo(n);
            l.find('[data-toggle="lightbox"]').magnificPopup({
                type: "image"
            })
        }
        if (a >= o) {
        ...
        if (o == 0) {
            ...
            $.removeCookie("cart", { path: '/' });
            updateCart();
            $("#total-cart").find("span").text(o.toString());
            return
        }
        r.find("span").text("Displaying " + a.toString() + " of " + o.toString() + " items");
        $("#total-cart").find("span").text(o.toString());
...
jQuery(function(e) {
    ...
    e(document).on("click", ".clear_cart", function(t) {
        t.preventDefault();
        unsetCookie();
        location.reload(); 
    });

    e(document).on("click", ".reorder", function(t) {
        t.preventDefault();
        var n = e(this),
            r, hello = {};
        orderId = n.closest('td').parent().find('.order_id').text();
        $.ajax(
        {
            // Post select to url.
            type : 'post',
            url : 'includes/previous_order.php',
            
            data : 
            {
                    'orderId' : orderId // the variable you're posting.
            },
    ...
    e(document).on("click", ".add-to-sample", function(t) {
        t.preventDefault();
        var n = e(this),
            r, i = {};
        r = n.closest(".add-cart");
        $item = n.closest(r.data("product"));
        var option_value = $("input[name='ad_option']:checked").val();
        i = {
            id: $item.data("product-id"),
            thumbnail: $item.find(r.data("thumbnail")).attr("src"),
            title: e.trim($item.find(r.data("title")).text()),
            url: $('#hidden_url').attr("href"),
            price: "0",
            qty: "1",
            ad_option : option_value
           
        };
        console.log($item);
        var s = $item.find(r.data("thumbnail")),
            o = e('.header-top [data-target="#sub-cart"]'),
            u = s.clone().offset({
                top: s.offset().top,
                left: s.offset().left
            }).css({
                ...
                var t = e(".cart-notification ul"),
                    n = e('<li><strong>"' + i.title + '"</strong> Added to Your Sample Cart Succesfully.</li>').hide();
    ...
    e(document).on("click", ".add-to-cart", function(t) {
        ...
        if ($("input[name='ad_option']:checked").length == 0)
        {
            bootbox.dialog({
              message: "Please choose if the chosen inventory is open for Advertisement. Click yes or no to proceed.",
              title: "Advertisement Option",
              buttons: {
                yes: {
                  label: "Yes",
                  className: "btn-success",
                  callback: function() {
                    $('input:radio[name="ad_option"]').filter('[value="yes"]').attr('checked', true);
                    /*$('.add-to-cart').click();*/
                  }
                },
                no: {
                  ...
                  callback: function() {
                      $('input:radio[name="ad_option"]').filter('[value="no"]').attr('checked', true);
                      /*$('.add-to-cart').click();*/
                  }
                }
              }
        ...
        var s = $item.find(r.data("thumbnail")),
            ...
            u = s.clone().offset({
                ...
                var t = e(".cart-notification ul"),
                    n = e('<li><strong>"' + i.title + '"</strong> Added to Your Cart Succesfully.</li>').hide();
                n.appendTo(t).slideDown(400, function() {
                    setTimeout(function() {
                        n.slideUp(400, function() {
                            e(this).remove()
    ...
    e(document).on("click", ".buy_now", function(t) {
        t.preventDefault();
